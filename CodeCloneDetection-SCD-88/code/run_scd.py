            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['eval_map']>best_acc:
                        ...
                        if args.save_task_adap:
                            model.encoder.save_adapter(args.output_dir, args.task_adap_name)
                        else:
                            model.encoder.save_pretrained(args.output_dir)      
                        
                        logger.info("Saving task adapter checkpoint to %s", args.output_dir)
...
def main():
    ...
    parser.add_argument("--save_task_adap", action='store_true',
                        help="Whether to save the task adapters separately")
    parser.add_argument(
        "--lang_adap_name",
        type=str,
        default="lang",
    ...
    parser.add_argument(
        ...
        type=str,
        default="lang",
        help="Pass the name of the task adapter to be trained",
    )
    parser.add_argument(
        "--lang_adapter_config",
        type=str,
        default="pfeiffer",
        help="Choose the lang_adapter config. Choices include [houlsby|pfeiffer|either+inv].",
    )
    parser.add_argument(
        "--task_adapter_config",
        type=str,
        default="pfeiffer",
        help="Choose the task_adapter config. Choices include [houlsby|pfeiffer|either+inv].",
    )
    parser.add_argument(
        "--drop_layers",
        nargs='+',
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to pretrained adapter or adapter identifier from adapterhub.ml.",
    )
    ...
    if args.train_adapter:
        lang_set = args.lang_adap_name or "mlm"
        ...
        # check for task based fine-tuning request
        if args.task_adap_name is not None:
            task = args.task_adap_name
            # resolve adapter configuration
            adapter_config = AdapterConfig.load(
                args.task_adapter_config,
                non_linearity=args.adapter_non_linearity,
                reduction_factor=args.task_adapter_reduction_factor,
            )
            
            # add the fresh task adapter
            model.add_adapter(task, config=adapter_config)
            
            # Stack adapters in MAD-X fashion
            model.active_adapters = ac.Stack(lang_set, task)
            # specify the adapters to be trained
            model.train_adapter([task])
        else:
            model.set_active_adapters([lang_set])
    else:
        if args.load_adapter is not None: # or adapter_args.load_lang_adapter:
            raise ValueError(
